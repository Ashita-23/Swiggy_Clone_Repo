...
// console.log(allRestaurant,"00")
    const topRated = allRestaurant?.filter((cards)=>cards?.data?.data?.avgRating > 3.5 );
   return topRated
}
    ...
    return(<>
        <div className="Counter-outer">
            ...
            <div className="search-box">
                <input type="text" placeholder="Search" className="search-input" value={inputText} onChange={(event)=>setInputText(event.target.value)} /> 
                <button type="submit" className="input-btn" ><i className="fa-solid fa-magnifying-glass"></i></button>
                </div>
            ...
            <div className="card-display-outer">
                        {
                            resturantList?.map((cards)=>{
                               ...
                               return(<> <RestaurantsCards resturantLists={cards} key={cards?.data?.data.id} /></>)
                            })
                        }
